...
char send_command[32] = "wait cmd";
bool send_cmd_flag = 0;
...
static int example_mqtt(int argc, const char *argv[])
{
    ...
    abort_exec = shell_signal(1, test_close_sendpic);
    ...
    while(1) {
        ...
        if(send_cmd_flag)
        {
            ...
            mqtt_publish(&client, topic, send_command, strlen(send_command) + 1, MQTT_PUBLISH_QOS_0);
