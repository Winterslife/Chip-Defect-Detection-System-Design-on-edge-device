...
extern char disp_face_result[32];

uint8_t sendbuf[128]; /* sendbuf should be large enough to hold multiple whole mqtt messages */
uint8_t recvbuf[128]; /* recvbuf should be large enough any whole mqtt message expected to be received */
 ...
 */
//static void test_close_face(int sig)
void test_close_face(int sig)
{
    if (test_sockfd)
    {
        close(test_sockfd);
...
static int example_mqtt(int argc, const char *argv[])
{
    ...
    abort_exec = shell_signal(1, test_close_face);
...
static void publish_callback_1(void** unused, struct mqtt_response_publish *published)
{
    ...
    sprintf(disp_face_result, "%s", topic_msg, strlen(topic_msg));
